...
| `provider-instances.yaml` | **NATIVE to this repo (not vendored).** The runner-side definition of every provider INSTANCE the three pipelines address. `capsule.dot`'s `critique`, `feature-capsule.dot`'s `critique` and `task-runner.dot`'s `critique_b` declare `llm_provider="luna"` -- a `config.providers[].id` entry in the operator's Amplifier settings, not a provider MODULE name -- and a GitHub-hosted runner has no such settings at all, so without this file the run is not degraded but REFUSED (the engine's issue-#155 startup preflight declines to start and names the node). **Contains no secret and must never contain one:** every credential is a `${VAR}` placeholder the engine expands from the process environment at load time, so the file on disk carries the literal placeholder text. Required repo secrets: `LUNA_API_KEY`, `LUNA_BASE_URL`. |
| `install_provider_instances.sh` | **NATIVE to this repo (not vendored).** Puts the file above in place, or refuses loudly before any budget is spent. Replaces the three per-workflow `OPENAI_API_KEY` preflights, which checked a credential the graphs no longer declare. It derives which instance ids to provision BY SCANNING THE GRAPHS and which credentials to demand BY SCANNING THE TEMPLATE, so neither list is written down twice. Three refusals are deliberate: it never writes a secret to disk; it never overwrites settings a self-hosted runner already carries (the other supported shape -- clobbering an operator's provider config would be exactly the silent substitution this pipeline refuses); and it never weakens the engine's own startup preflight to make a run start. `tests/test_no_live_gpt5_pins.py` holds the graphs and this file to each other. |
...
- **2026-09-07** -- **MODEL PINS ONLY; no graph shape changed.** All three
  ...
  - **Infra shipped with the pin, because the pin alone would refuse.**
    ...
    required: `LUNA_API_KEY`, `LUNA_BASE_URL`.** Until they exist, the three
  ...
  - `scrub_secrets.py`'s literal-credential watch list gained `LUNA_API_KEY`
    and `LUNA_BASE_URL`, and every scrub/scan/gate step passes them. A watch
