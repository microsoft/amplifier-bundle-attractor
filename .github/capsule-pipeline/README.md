...
| ~~`attractor-pipeline-dual.yaml`~~ | **REMOVED (currency sweep, engine 0.2.0).** Was a multi-provider (Anthropic + OpenAI) base bundle unconditionally mounted by both workflows so `task-runner.dot`'s `critique_b` and `capsule.dot`'s `critique` (both hard-declaring a second model family) resolved to the right one (issue #155). The dot-runner engine now resolves per-node `llm_provider` natively -- no base-bundle mount is needed for a node's declared provider to be honored -- so this workaround bundle was dead weight; both workflows had already stopped setting `ATTRACTOR_PIPELINE_BUNDLE`/`DOT_RUNNER_BUNDLE` (both forms are gone from the CLI as of 0.2.0 anyway -- see their own "WORKER-SURFACE FLIP" comments). Each workflow's preflight is retained -- the dual-family credential requirement did not go away, only the bundle-mount plumbing that used to deliver it. As of 2026-09-07 that preflight provisions the `luna` provider INSTANCE instead of checking `OPENAI_API_KEY`; see `provider-instances.yaml` below. |
| `provider-instances.yaml` | **NATIVE to this repo (not vendored).** The runner-side definition of every provider INSTANCE the three pipelines address. `capsule.dot`'s `critique`, `feature-capsule.dot`'s `critique` and `task-runner.dot`'s `critique_b` declare `llm_provider="luna"` -- a `config.providers[].id` entry in the operator's Amplifier settings, not a provider MODULE name -- and a GitHub-hosted runner has no such settings at all, so without this file the run is not degraded but REFUSED (the engine's issue-#155 startup preflight declines to start and names the node). **Contains no secret and must never contain one:** every credential is a `${VAR}` placeholder the engine expands from the process environment at load time, so the file on disk carries the literal placeholder text. Credentials it references, and their two different shapes (owner ruling, 2026-09-07): **`OPENAI_API_KEY` is a repo SECRET and is REQUIRED** -- the installer refuses, naming it, when it is absent; **`OPENAI_BASE_URL` is a repo VARIABLE and is OPTIONAL** -- an endpoint URL rather than a credential, so when it is unset the installer OMITS the `base_url` key entirely and the `provider-openai` module's own default endpoint applies (and the step log says so). The header's `optional-vars:` directive is the machine-readable half of that split; the installer reads it, so prose and behaviour cannot drift apart. |
| `install_provider_instances.sh` | **NATIVE to this repo (not vendored).** Puts the file above in place, or refuses loudly before any budget is spent. Replaces the three per-workflow `OPENAI_API_KEY` preflights, which checked a credential the graphs no longer declare. It derives which instance ids to provision BY SCANNING THE GRAPHS, which credentials to demand BY SCANNING THE TEMPLATE, and which of those are OPTIONAL from the template's own `optional-vars:` directive, so no list is written down twice. An optional var that is unset is not a refusal: its whole `key: ${VAR}` line is left out of the installed file so the provider module's default applies, and the step log names the omission -- "the operator chose the default" and "the endpoint silently went missing" must not read the same in a log. Five refusals are deliberate: it never writes a secret to disk; it never overwrites settings a self-hosted runner already carries (the other supported shape -- clobbering an operator's provider config would be exactly the silent substitution this pipeline refuses); it never weakens the engine's own startup preflight to make a run start; it never omits a REQUIRED credential's line to get a run started; and it refuses rather than guessing when an unset optional var appears in a shape it cannot surgically remove (an unexpandable `${VAR}` is left VERBATIM by the engine, which in a `base_url` would aim the run at a literal host). `tests/test_no_live_gpt5_pins.py` holds the graphs and this file to each other; `tests/test_provider_base_url_optional.py` runs this script for real, in a throwaway HOME, on both the endpoint-set and endpoint-absent branches. |
...
`capsule.dot`, `task-runner.dot`, and
...
critique needs a second model-provider key). That gap was originally closed
at the **workflow** level (`capsule-implement.yml` unconditionally setting
`ATTRACTOR_PIPELINE_BUNDLE` to the vendored `attractor-pipeline-dual.yaml`,
mirrored by `capsule-specify.yml` for `capsule.dot`'s `critique` node) --
...
credential requirement is real regardless of which mechanism delivers it.
...
- **2026-09-07 (later the same day)** -- **CREDENTIAL SHAPE ONLY; no graph, no
  pin, and no refusal path changed.** Two owner rulings about
  `OPENAI_BASE_URL`, mirrored expression-for-expression in
  `amplifier-bundle-dot-runner`:
  1. **It is OPTIONAL.** Absent, the `luna` instance runs on the
     `provider-openai` module's own default endpoint -- "the default should
     have been fine". `install_provider_instances.sh` no longer demands it: it
     reads an `optional-vars:` directive out of `provider-instances.yaml`,
     drops the whole `base_url: ${OPENAI_BASE_URL}` line from the installed
     file when the variable is unset, and SAYS SO in the step log. What still
     refuses, untouched: a missing `OPENAI_API_KEY`, and an instance the graphs
     address that nothing defines (the issue-#155 / EXTENSIONS Sec 36
     guarantee that no run starts unable to reach its second judge).
  2. **It is NOT a secret.** It is an endpoint URL, set as a repository
     VARIABLE on both repos, so all 14 assignments across the three workflows
     read `${{ vars.OPENAI_BASE_URL || secrets.OPENAI_BASE_URL }}` -- variable
     first, secret kept as a fallback so an operator who stored it as a secret
     before the ruling is not broken.
  - **This supersedes the "New repo secrets required: `OPENAI_API_KEY`,
    `OPENAI_BASE_URL`" line in the entry below**, and the matching sentence in
    `docs/designs/REVIEW-pipelines-2026-09.md` Sec 4.4's addendum (which has an
    addendum of its own now). Both are left VERBATIM: they record what was
    required then. One secret is required now.
  - The scrubber keeps `OPENAI_BASE_URL` on its literal watch list -- an
    endpoint's value carries no debugging value in evidence -- and an EMPTY
    value is a no-op there, not a scrub. That is load-bearing rather than
    incidental: an empty literal matches at every position in every file, so an
    unguarded watch list would not leak the evidence tree, it would destroy it,
    on exactly the configuration this ruling makes normal.
  - New guard: `tests/test_provider_base_url_optional.py`, RED before the
    change (5 of 11). It runs the real installer in a throwaway HOME with an
    explicitly built environment (a developer exporting `OPENAI_BASE_URL`
    cannot colour it), covers unset AND set-but-empty -- the shape CI actually
    produces, since Actions sets the env entry to `""` rather than dropping it
    -- proves the omission is surgical (installed file == template minus
    exactly that one line, byte for byte), proves neither the URL nor the key
    is ever expanded onto disk, pins the workflow expression, and carries a
    positive control for the scrubber so the empty-value case cannot pass
    vacuously.

- **2026-09-07** -- **MODEL PINS ONLY; no graph shape changed.** All three
  pipelines' judgment nodes moved off the OpenAI generation-range glob and onto
  a configured provider INSTANCE: `capsule.dot`'s `critique`,
  `feature-capsule.dot`'s `critique` and `task-runner.dot`'s `critique_b` now
  declare `llm_provider="luna", llm_model="gpt-5.6-luna",
  reasoning_effort="high"`. Standing owner policy: gpt-5 is not a live choice;
  address a `-terra` or `-luna` instance instead.
  - **Why the model id is concrete rather than another glob.** A glob is
    resolved by `unified_llm.resolve_latest_for`, whose SDK adapters cover the
    anthropic/openai/gemini triad only; it has no adapter for an instance id
    and fails loud with `no adapter found for provider 'luna'` -- the same
    error text `task-runner.dot`'s own #155 timeline records. A concrete id is
    returned unchanged with no catalog call
    (`backend.py::_resolve_concrete_model`).
  - **Why luna, at that effort.** node-matrix run `20260907T164521Z-axes`
    measured both instances on the capsule `author` workload under the
    coding-agent worker, all rows PASS `red_ok`:
    luna/`gpt-5.6-luna`/high **4.3 min / $0.08**;
    terra/`gpt-5.6-terra`/high 8.0 min / $7.39;
    openai/`gpt-5`/high 43.6 min / $7.83. The effort is the one measured, not
    a guess.
  - **Infra shipped with the pin, because the pin alone would refuse.**
    `provider-instances.yaml` + `install_provider_instances.sh` (see Files),
    wired into all three workflows' preflight steps. **New repo secrets
    required: `OPENAI_API_KEY`, `OPENAI_BASE_URL`.** Until they exist, the three
    capsule workflows refuse at their preflight step and name both. That is
    the intended behavior, not a regression: `docs/designs/REVIEW-pipelines-2026-09.md`
    §4.4 states that a terra/luna pin shipped without its infra is refused by
    the startup preflight, which is why it cannot be smuggled in "to see if
    it works".
  - Independence -- the reason a second judge exists at all -- is unchanged:
    `luna` is an instance of `provider-openai`, still a different family from
    the Anthropic maker. What changed is which openai-family model, and which
    credential the run needs.
  - `scrub_secrets.py`'s literal-credential watch list gained `OPENAI_API_KEY`
    and `OPENAI_BASE_URL`, and every scrub/scan/gate step passes them. A watch
    list that lags the credentials a run actually holds is the 2026-08 leak
    incident waiting to recur under a different variable name.
  - Guarded by `tests/test_no_live_gpt5_pins.py`, which reads model SELECTION
    (node attrs through the repo's stdlib DOT reader, plus `model_stylesheet`
    rule bodies) rather than text -- because the two worst shapes,
    a resolving glob and a bare `llm_provider="openai"` falling to the
    engine default `gpt-5.*[0-9]`, both contain no "gpt-5" to grep for.
  - Earlier entries in this log quote the old `llm_model="gpt-[5-9]*"` pin.
    They are left VERBATIM: they are the record of what shipped then.
