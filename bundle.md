  ...
  attractor-pipeline-runner:
    ...
    session:
      orchestrator:
        ...
        config:
          # default_provider: the provider used by nodes that declare no llm_provider.
          # This USED to be a hardcoded "anthropic" literal in engine code. A provider
          # literal in the engine is POLICY, and it is wrong for any bundle that does
          # not mount it -- so it lives here, at the edge, where the bundle owns it.
