...
> **DEVIATION — shipped without the message enrichment** (recorded post-adversarial-review, #228). The hard-fail itself shipped exactly as designed; the *"extended to name the checkpoint file, the recorded node and outcome status"* clause did **not**. A resume-hop dead-end emits the generic `_no_matching_edge_reason()` message — the same one the fresh path emits — naming the node and `error_type=no_matching_edge`, with no checkpoint provenance.
>
> **Enrichment deferred, deliberately.** The generic error is already loud, already fail-closed, and already terminates the run; the enrichment would add provenance the operator has from two independent sources anyway (they invoked `attractor resume <run_dir>` themselves, and `trace.jsonl` carries `PIPELINE_RESUME` with `checkpoint_node` immediately before the `PIPELINE_ERROR`). Against that near-zero diagnostic delta, implementing it means adding resume-hop state to the shared `_run_loop` edge-selection failure path — the one path AC-4 exists to keep byte-identical on fresh runs — and getting a second one-shot flag right across that loop's early-`continue` branches, where a mis-cleared flag would make a *later* failure falsely claim to be the resume hop. Wrong trade for a message decoration. If it is ever built, the honest shape is a local `is_resume_hop = resume_outcome is not None` read at the top of the loop body (no new engine state, self-clearing) plus a defaulted kwarg on `_no_matching_edge_reason()`, and it needs its own resume-hop-dead-ends test.
