...
and dispositioned in `SPEC_CONFORMANCE.md` (`CAL-*` rows).

Keeping two spec files that disagree is worse than keeping one: a reader has no way to tell which
is normative, and a citation to `specs/coding-agent-loop-spec.md:<line>` silently means nothing after the next upstream sync.
